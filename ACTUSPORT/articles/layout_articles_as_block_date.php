 ...
 */
Class Layout_articles_as_block_date extends Layout_interface {
	...
	function layout($result) {
		...
		while($item = SQL::fetch($result) ) { // && $cpt<30
			...
			if(is_object($overlay))
				$ddate = $overlay->get_text('block_date', $item);
			...
			foreach($images = Layout_articles_as_block_date::list_by_date_for_anchor_asc('article:'.$item['id'], 0, 1, 'raw') as $image) {
