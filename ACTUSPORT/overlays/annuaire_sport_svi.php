 ...
 */
class Annuaire_sport_svi extends Overlay {
	...
	function get_fields($host) {
		...
		$label = i18n::s('Sport').' *';
		$input = Annuaire_sport_svi::get_listOption(array('une', 'keywords','hebdomadaire','mensuel'), isset($host['id'])?Members::list_anchors_for_member('article:'.$host['id']):null,'category:10');
		...
		include_once($context['path_to_root'].'categories/categories.php');
		include_once($context['path_to_root'].'shared/members.php');
		$label = i18n::s('R&eacute;gion').' *';
		$input = Annuaire_sport_svi::get_listOption(array('une', 'keywords','hebdomadaire','mensuel'), isset($host['id'])?Members::list_anchors_for_member('article:'.$host['id']):null,'category:111');
		$hint = BR.i18n::s('Choisissez une r&eacute;gion');
		$fields[] = array($label, $input, $hint);

		// ville et département
		$label = i18n::s('Code postal - Ville').' *';
 		...
 		$stat = Locations::stat_for_anchor('article:'.$host['id']);
  		if(!isset($host['id']) || !$stat || ($stat['count']==0)) { // en modif on utilise yacs normal
			$input = '<input id="cpostal" name="cpostal"  size=6 value="'.encode_field(isset($this->attributes['cpostal'])?$this->attributes['cpostal']:'').'">';
			...
			$hint = i18n::s('en toute lettre. <strong>Pas d\'abr&eacute;viation !</strong>');
			$fields[] = array($label, $input, $hint);
			  /*
	
			$context['text'] .= '<script src="http://maps.google.com/maps?file=api&amp;v=2&amp;key='.$context['google_api_key'].'" type="text/javascript"></script>'."\n";
			...
			. 'if(!validateDocumentPost(container)) return false;'."\n";
			  ...
			  $context['page_footer'] .= '  container.geo_place_name.value = trim(container.ville.value + " ") + container.cpostal.value,'."\n";
	...
	function get_label($name, $action='view') {
		...
		// edit command
		case 'edit_command':
			return i18n::s('Modifier un club');
			break;

		// new command
		case 'new_command':
			return i18n::s('Ajouter un  club');
			break;

		// publish command
		case 'publish_command':
			return i18n::s('publier un club');
			break;

		case 'introduction':
			return i18n::s('adresse');
			break;
		...
		case 'page_title':
			...
			case 'edit':
				return i18n::s('Modification du club');

			case 'delete':
				return i18n::s('Effacer un club');

			case 'new':
				return i18n::s('Nouveau club sportif');
	...
	function &get_live_introduction($host=NULL) {
		$text = Codes::beautify_introduction($host['introduction']);
		return $text;
	}
	...
	function get_text($variant='view', $host=NULL) {
		...
		case 'introduction':
			...
			$list_category = Annuaire_sport_svi::list_category_by_title_for_members('article:'.$host['id'],10, 0,1,'raw');
		  	$categtitle = '';	
			...
			$list_category = Annuaire_sport_svi::list_category_by_title_for_members('article:'.$host['id'],111, 0,1,'raw');
		  	$categtitle = '';	
			foreach($list_category as $id=>$categ) {
				//if (strlen($categ['thumbnail_url'])> 0)
		...
		case 'trailer':
		  ...
		  case 'view' :
			  ...
			  if (strlen($this->attributes['link_url'])>0) 
				  $text .= '<p><b> site internet :  '.$this->attributes['link_url']."</b></p>\n";
			
			$text = Codes::beautify($text) . Codes::beautify($host['description']) ;
			break;
		default:
			return $text;
		}
...
/*echo "<script language='javascript'>alert('".$_FILES['vignette_file']['tmp_name']."');</script>";*/
		...
		$this->attributes['cpostal'] = $fields['cpostal'];
