 ...
 */
class Annuaire_sport extends Overlay {
	...
	function get_fields($host) {
		...
		$label = i18n::s('Sport').' *';
		$input = Annuaire_sport::get_listOption(array('une', 'keywords','hebdomadaire','mensuel'), isset($host['id'])?Members::list_anchors_for_member('article:'.$host['id']):null,'category:10');
		...
		$input = Annuaire_sport::get_listOption(array('une', 'keywords','hebdomadaire','mensuel'), isset($host['id'])?Members::list_anchors_for_member('article:'.$host['id']):null,'category:111');
	...
	function get_text($variant='view', $host=NULL) {
		...
		case 'introduction':
			...
			$list_category = Annuaire_sport::list_category_by_title_for_members('article:'.$host['id'],10, 0,1,'raw');
		  	$categtitle = '';	
			...
			$list_category = Annuaire_sport::list_category_by_title_for_members('article:'.$host['id'],111, 0,1,'raw');
		  	$categtitle = '';	
			foreach($list_category as $id=>$categ) {
				//if (strlen($categ['thumbnail_url'])> 0)
		...
		case 'trailer':
		  ...
		  case 'view' :
			...
			$text = Codes::beautify($host['description']) . Codes::beautify($text);
			break;
		default:
			return $text;
		}
