 ...
 */
class Agenda_sport extends Overlay {
	...
	function get_fields($host,$field_pos=NULL) {
		...
		$input = Agenda_sport::get_listOption(array('une', 'keywords','hebdomadaire','mensuel'), isset($host['id'])?Members::list_anchors_for_member('article:'.$host['id']):null,'category:10');
		...
		include_once($context['path_to_root'].'shared/members.php');
		$label = i18n::s('Cat&eacute;gories').' *';
		$input = Agenda_sport::get_listOption(array('une', 'keywords','hebdomadaire','mensuel'), isset($host['id'])?Members::list_anchors_for_member('article:'.$host['id']):null,'category:111');
		$hint = BR.i18n::s('Choisissez une r&eacute;gion');
		$fields[] = array($label, $input, $hint);

		// dates animation
		$label = i18n::s('Date(s) de l\'animation').' *';
		$input = Skin::build_input_time('date_debut', isset($this->attributes['date_debut'])?$this->attributes['date_debut']:'', 'date');
		$input .= '<span style="margin-left: 20px;">Fin (facultatif): '.Skin::build_input_time('date_fin', isset($this->attributes['date_fin'])?$this->attributes['date_fin']:'', 'date').'</span>';
	...
	function &get_text($variant='view', $host=NULL) {
		...
		case 'trailer':
		  ...
		  case 'jour':
			...
			$list_category = Agenda_sport::list_category_by_title_for_members('article:'.$host['id'],10, 0,1,'raw');
		  	$categtitle = '';
			foreach($list_category as $id=>$categ) {
					$text .= 'sport <span>'.$categ['title'].'</span>'."\n";
			}
			// CHL recherche de la region
			$list_category = Agenda_sport::list_category_by_title_for_members('article:'.$host['id'],111, 0,1,'raw');
		  	$categtitle = '';
			foreach($list_category as $id=>$categ) {
					$text .= 'r&eacute;gion <span>'.$categ['title'].'</span>'."\n";
			}
		  ...
		  case 'view' :
			...
			foreach($list_category as $id=>$categ) {
				if (strlen($categ['thumbnail_url'])> 0)
					$text .= BR. 'sport <img src="'.$categ['thumbnail_url'].'" alt="sport" />'."\n";
				else
					$text .= BR. 'sport :  <span>'.$categ['title'].'</span>'."\n";
			}
			$list_category = Agenda_sport::list_category_by_title_for_members('article:'.$host['id'],111, 0,1,'raw');
			foreach($list_category as $id=>$categ) {
					$text .= BR .'r&eacute;gion : <span>'.$categ['title'].'</span>'."\n";
			}
			$text .= BR;
...
google_ad_client = "ca-pub-8308574989015080";
/* sport */
google_ad_slot = "9005425418";
google_ad_width = 468;
google_ad_height = 60;
//-->
</script>
...
/*echo "<script language='javascript'>alert('".$_FILES['vignette_file']['tmp_name']."');</script>";*/
	 ...
	 */
	function remember($variant, $host,$reference) {
	...
	Articles::put($host);
		//categories
		// CHL assignation des catŽgories corrigŽes le 30/08/11
		Members::unlink_for_reference('article:'.$host['id']);
		foreach($_REQUEST as $key => $value) {
		  if (substr($key, 0, 9) == 'category:') {
			Members::assign($value, 'article:'.$host['id']);
		//Logger::debug('CHL KEY:'.$key.' value:'.$value.'article:'.$host['id'],'');
		  }
		}
...
//
	...
	function &list_category_by_title_for_members($anchor, $category, $offset=0, $count=10, $variant=NULL) {
		...
		$where = " categories.anchor='category:".$category."'";
